...
const { normalize: normalizeAddress } = require('@fksyuan/eth-sig-util')

const SimpleKeyring = require('@fksyuan/eth-simple-keyring')
const HdKeyring = require('@fksyuan/eth-hd-keyring')
...
class KeyringController extends EventEmitter {
  ...
  async getAccounts () {
    ...
    addrs = addrs.map(normalizeAddress)
  ...
  async _updateMemStoreKeyrings () {
    const keyrings = await Promise.all(this.keyrings.map(this.displayForKeyring.bind(this)))
